...
function buildSampleSources(): Partial<Record<number, AVPlaybackSource>> {
  ...
  // Uncomment each line only after the file exists at that path.
...
async function playSource(
  ...
  try {
    ...
    console.log('playSource error:', error);
    return null;
  }
}
...
export async function playNote(midi: number, volume: number = 0.9): Promise<void> {
  ...
  if (!nearest) {
    console.log('No sample sources loaded for playNote. Midi:', midi);
    return;
  }
...
export async function playInterval(
  ...
  if (harmonic) {
    ...
    await wait(600);
    await playNote(rootMidi + semitones, 0.9);
  }
}
