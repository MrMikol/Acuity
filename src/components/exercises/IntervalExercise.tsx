 ...
 *
 * Flow:
 *   1. Press Play → interval plays
 *   2. Tap your answer from the option buttons
 *   3. See immediate feedback
 *   4. Press Next → repeat
 *
 * Parent screen decides what to do with completed results.
 */
...
export const IntervalExercise: React.FC<IntervalExerciseProps> = ({
  ...
  const handlePlay = useCallback(async () => {
    ...
    try {
      await playInterval(rootMidi, question.interval.semitones, false);
    } finally {
      setTimeout(() => {
        setPressedKeys(new Set());
        setIsPlaying(false);
    ...
    return (
      <View style={styles.resultContainer}>
        ...
        <Text
          style={[
            styles.resultSub,
            { color: passed ? colors.sage : colors.blush },
          ]}
        >
          {passed
...
function createStyles(colors: ReturnType<typeof useTheme>) {
  return StyleSheet.create({
    ...
    playBtnActive: {
      backgroundColor: `${colors.slate}CC`,
    },
    playBtnText: {
